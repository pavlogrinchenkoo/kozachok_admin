...
abstract class _Calculator {
  ...
  Rect moveRect(Rect original, double deltaX, double deltaY, Rect imageRect) {
    ...
    if (original.left + deltaX < imageRect.left) {
      newDeltaX = (original.left - imageRect.left) * -1;
    }
    if (original.right + deltaX > imageRect.right) {
      newDeltaX = imageRect.right - original.right;
    }
    if (original.top + deltaY < imageRect.top) {
      newDeltaY = (original.top - imageRect.top) * -1;
    }
    if (original.bottom + deltaY > imageRect.bottom) {
      newDeltaY = imageRect.bottom - original.bottom;
    }
    print("${original.top + deltaY} :TOP");
    print("${original.bottom + deltaY} :BOTTOM");
    print("${original.width} :WIDTH");
    print("${original.height + deltaY} :height");
    return Rect.fromLTWH(
      original.left + newDeltaX,
      original.top + deltaY,
      original.width,
      original.height,
    );
